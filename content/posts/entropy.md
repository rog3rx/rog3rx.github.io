...
如果你认为硬币是公平的（$q = 0.5$），但实际上硬币有 90% 的概率正面朝上（$p = 0.9$），你面对真实结果时就会比预期更加"惊讶"。随着你不断更新信念 $q$ 以使之更接近 $p$，平均惊讶程度（即交叉熵）会逐渐降低。当 $q$ 与 $p$ 完全一致时，交叉熵达到最小值，等于系统固有的不确定性（熵）。

### 2.4 不对称性

交叉熵是不对称的，因为真实分布 $P$ 决定了结果发生的频率，而我们估计的 $Q$ 则塑造了我们对结果的惊讶感。
...
我们如何估计这两个概率分布的距离？
我们有个直观的感受，如果第二枚硬币的正面概率为 0.55，那么第二枚硬币明显更接近公平的硬币，也就是说概率分布更加的近似。但如果是正面概率为0.9的硬币，我们也可以很快感受到和公平硬币的差距。
...
$$\frac{n_H}{n} \to p,\quad \frac{n_T}{n} \to 1-p$$
...
$$D_{\mathrm{KL}}(P \| Q) = \sum_{i} P(i)\,\ln\Bigl(\tfrac{P(i)}{Q(i)}\Bigr) = \sum_{i} P(i)\,\ln\Bigl(\tfrac{1}{Q(i)}\Bigr) \quad (\text{因为对错误类别 }P(i)=0)$$
